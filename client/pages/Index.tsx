...
export default function Index() {
  ...
  const {
    ...
    queryFn: () =>
      listEmails({ q, priority: priority === "all" ? undefined : priority }),
        ...
        <ErrorBoundary>
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <RefreshCw
                      className={
                        "h-4 w-4" + (sync.isPending ? " animate-spin" : "")
                      }
                    />
                  </Button>
                </div>
                ...
                <div className="divide-y rounded-md border">
                  {isLoading || isFetching ? (
                    ...
                    emails.map((e) => (
                      <button
                        key={e._id}
                        className="w-full text-left p-3 hover:bg-muted/50 focus:bg-muted/50"
                        onClick={() => onOpen(e)}
                      >
                        <div className="flex items-center gap-2">
                          <Badge
                        ...
                        <div className="text-sm line-clamp-2 mt-1 text-muted-foreground">
                          {e.snippet}
                        </div>
                      </button>
                    ))
          ...
          <div className="lg:col-span-2">
            {!selected ? (
              ...
              <EmailDetail
                email={selected}
                onRefresh={() => qc.invalidateQueries({ queryKey: ["emails"] })}
              />
            )}
          </div>
        </ErrorBoundary>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="content" className="mt-4 space-y-2">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html:
                  email.bodyHtml ||
                  `<pre class=\"whitespace-pre-wrap\">${email.bodyText || email.snippet}</pre>`,
              }}
            />
          </TabsContent>
          <TabsContent value="ai" className="mt-4 space-y-3">
            ...
            <div className="grid gap-3">
              {(suggest.data?.suggestions || email.aiSuggestions || []).map(
                (s, i) => (
                  <div
                    key={i}
                    className="rounded-md border p-3 text-sm bg-muted/30 whitespace-pre-wrap"
                  >
                    {s}
                  </div>
                ),
          ...
          <TabsContent value="schedule" className="mt-4 space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                onClick={() => slotsQuery.mutate()}
                disabled={slotsQuery.isPending}
              >
            ...
            {slotsQuery.data && (
              <div className="text-xs text-muted-foreground">
                Timezone: {slotsQuery.data.timezone}
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={!selectedSlot || createEventMut.isPending}
              >
